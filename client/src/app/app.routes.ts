...
import { AuthGuard, AdminGuard, adminOrdersRedirectToDashboardGuard } from './services/auth.guard';
...
const langRoutes = languages.map(lang => {
  return [
    ...
    { path: lang + '/cart', loadChildren: () => import('./modules/cart/cart.module').then(m => m.CartModule) },
    {
      ...
      canActivate: [AuthGuard, adminOrdersRedirectToDashboardGuard],
    },
    {
      path: lang + '/account',
      loadComponent: () =>
        import('./components/account/account-page.component').then((m) => m.AccountPageComponent),
