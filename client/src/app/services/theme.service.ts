...
export class ThemeService {
  ...
  setCSSVariable(color: string, type: string): void {
    const root = this.document.documentElement;
    root.style.setProperty(`--${type}`, color);
    if (type === 'primary-color' && color) {
      this.syncMaterialPrimaryTokens(color);
    }
  }

  /** Alinea botones Material (filled/protected) con el color primario del tema en runtime. */
  private syncMaterialPrimaryTokens(hex: string): void {
    const root = this.document.documentElement;
    root.style.setProperty('--mdc-filled-button-container-color', hex);
    root.style.setProperty('--mdc-protected-button-container-color', hex);
    root.style.setProperty('--mdc-filled-button-label-text-color', '#ffffff');
    root.style.setProperty('--mdc-protected-button-label-text-color', '#ffffff');
    root.style.setProperty('--mat-app-primary', hex);
  }
